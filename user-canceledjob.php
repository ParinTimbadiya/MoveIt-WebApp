...
<section class="home-section">
  ...
  <div>
    ...
    <h1>cancled job</h1>
    <?php
    $canceledjobquery = "SELECT * FROM jobs WHERE userid = '$UserId' AND IsCanceled = '1' AND EXISTS (SELECT JobId FROM bids WHERE bids.JobId = jobs.Id AND IsAccepted = 'True');";
    $canceledJobResult = $connection->query($canceledjobquery);
    $canceledjobrows = $canceledJobResult->fetch_all(MYSQLI_NUM);
    ?>
        ...
        <div class="table-responsive m-t-40">
          <table style="text-align: center;" id="myTable" class="table table-bordered table-striped">
            ...
            foreach ($canceledjobrows as $r4) {
              ...
              $sql = "SELECT Name FROM `cities` WHERE `Id` = '$r4[2]'";
              $result = $connection->query($sql);
              $FromCity = $result->fetch_assoc()['Name'];

              $sql = "SELECT Name FROM `cities` WHERE `Id` = '$r4[4]'";
              $result = $connection->query($sql);
              $ToCity = $result->fetch_assoc()['Name'];

              $queryForBids = "SELECT * FROM bids WHERE jobid = '$r4[0]' AND isAccepted = 'True'";
              $BidsRecord = $connection->query($queryForBids);
              $rowsOfBids = $BidsRecord->fetch_all((MYSQLI_NUM));
            ?>
              <tbody>
                <tr>
                  <td><img src="./assets/uploads/images/<?= $r4['6'] ?>" style="max-width: 100px;"></td>
                  <td><?= $FromCity ?></td>
                  <td><?= $r4[3] ?></td>
                  <td><?= $ToCity ?></td>
                  <td><?= $r4[5] ?></td>
